...
import { Card, CardBody, CardHeader, Input, Textarea, Button, Select, SelectItem, Switch, Spinner } from "@nextui-org/react";
...
function EditInstructorCourseContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!courseId) return;
    try {
      const payload = { id: courseId, ...formData, price: Number(formData.price) } as any;
      ...
      <Card>
        ...
        <CardBody className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Textarea label="Description" name="description" value={formData.description} onChange={handleChange} minRows={4} isRequired variant="bordered" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Select label="Level" selectedKeys={[formData.level]} onChange={(e) => setFormData({ ...formData, level: e.target.value })} variant="bordered">
            ...
            <Input label="Category" name="category" value={formData.category} onChange={handleChange} isRequired variant="bordered" />
